...
static int parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    // delay options
    ("delay.time", bpo::value<std::size_t>(&args->delay_time)->default_value(0), "Time UE waits before attempting to RACH in ms")
    ;
  ...
  if (vm.count("delay.time")) {
    printf("found delay.time in CLI args\n");
  }

  // copy over delay time
  args->phy.delay_time    = args->delay_time;
  args->stack.delay_time  = args->delay_time; 
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "Using delay of " << args.delay_time << "\n";
