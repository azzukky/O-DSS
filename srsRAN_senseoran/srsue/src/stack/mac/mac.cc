...
{
  return mac::init_delay(phy, rlc, rrc, 0);
}

bool mac::init_delay(phy_interface_mac_lte* phy, rlc_interface_mac* rlc, rrc_interface_mac* rrc, std::size_t delay_time_)
{
  phy_h = phy;
  rlc_h = rlc;
  rrc_h = rrc;
  ...
  ra_procedure.init_delay(phy_h, rrc, &uernti, &timer_alignment, &mux_unit, &task_sched, delay_time);
