...
{
  ra_proc::init_delay(phy_h_, rrc_, rntis_, time_alignment_timer_, mux_unit_, task_sched_, 0);
}

// Initializes memory and pointers to other objects
void ra_proc::init_delay(phy_interface_mac_lte*               phy_h_,
                   rrc_interface_mac*                   rrc_,
                   ue_rnti*                             rntis_,
                   srsran::timer_handler::unique_timer* time_alignment_timer_,
                   mux*                                 mux_unit_,
                   srsran::ext_task_sched_handle*       task_sched_,
                   std::size_t                          delay_time_)
{
  phy_h      = phy_h_;
  rntis      = rntis_;
  mux_unit   = mux_unit_;
...
void ra_proc::state_pdcch_setup()
{
  ...
  if (info.is_transmitted) {
    ...
    // could be that RA_TTI is the rach occasion?
    ra_rnti = 1 + (ra_tti % 10) + (10 * info.f_id);
    rInfo("seq=%d, ra-rnti=0x%x, ra-tti=%d, f_id=%d", sel_preamble.load(), ra_rnti, info.tti_ra, info.f_id);
    srsran::console(
        "Random Access Transmission: seq=%d, tti=%d, f_id=%d, ra-rnti=0x%x\n", sel_preamble.load(), info.tti_ra, info.f_id, ra_rnti);
...
void ra_proc::resource_selection()
{
  ...
  if (preambleIndex > 0) {
    ...
    if (sel_group == RA_GROUP_A) {
      if (rach_cfg.nof_groupA_preambles > 0) {
        ...
        sel_preamble = (rand() + delay_time) % rach_cfg.nof_groupA_preambles;
      } else {
      ...
      if (nof_groupB_preambles) {
        ...
        sel_preamble = rach_cfg.nof_groupA_preambles + (rand() + delay_time) % nof_groupB_preambles;
      } else {
  ...
  printf("Selected preambleIndex=%d maskIndex=%d GroupA=%d, GroupB=%d\n",
         sel_preamble.load(),
         sel_maskIndex,
