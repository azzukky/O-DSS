...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("scheduler.sched_save_path", bpo::value<string>(&args->stack.mac.sched.sched_save_path)->default_value("/tmp/scheduler_metrics.bin"), "Scheduler metrics save path")
    ...
    ("expert.rrc_command_path", bpo::value<std::string>(&args->general.rrc_command_path), "Command path that will be used to signal a handover should be executed")
    ...
    ("ric.agent.iq_save_path",bpo::value<string>(&args->phy.save_path), "RIC IQ samples save path")
  ...
  if (vm.count("log.all_level")) {
    ...
    args->ric_agent.enb_id = args->enb.enb_id;
#endif
...
int main(int argc, char* argv[])
{
  ...
  cout << "Found Agent command path: " << args.general.rrc_command_path << endl;
  cout << "Found Sched metrics path: " << args.stack.mac.sched.sched_save_path << endl;
