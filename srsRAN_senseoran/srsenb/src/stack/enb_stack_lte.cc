...
{
  //printf("Got LTE stack metrics call\n");
  ...
  if (ret.has_value()) {
    // wait for result
    //printf("LTE STACK: Has a value there, waiting...\n");
    *metrics = pending_stack_metrics.pop_blocking();
    //printf("Collected sample metrics: num_ues: %lu\n", metrics->mac.ues.size());
    //if(metrics->mac.ues.size() != 0) {
      //printf("identified a UE: tx_pkts %d\n", metrics->mac.ues[0].tx_pkts);
    //}
    return true;
  }
  //printf("LTE STACK DOESNT HAVE A VALUE IN THE WORK QUEUE!!!!\n");
  return false;
}
