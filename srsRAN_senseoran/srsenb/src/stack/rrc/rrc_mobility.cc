...
//! Method called whenever the eNB receives a MeasReport from the UE. In normal situations, an HO procedure is started
void rrc::ue::rrc_mobility::handle_dummy_report(uint32_t target_enb)
{
  ...
  //auto              obj_it = srsran::find_rrc_obj_id(measobj_list, measid_it->meas_obj_id);
  //meas_ev.meas_obj         = &(*obj_it);
  meas_ev.target_eci       = target_enb;
  meas_ev.direct_fwd_path  = false;

  // NOTE: From now we just choose the strongest.
  printf("!!!!!!!!!! TRIGGERING HANDOVER !!!!!!!!!!\n");
  trigger(meas_ev);
}
...
void rrc::ue::rrc_mobility::s1_source_ho_st::enter(rrc_mobility* f, const ho_meas_report_ev& ev)
{
  ...
  //make check to see if meas_obj is null, in which case we provide a dummy measurement report ID
  //if(report.meas_obj == nullptr) {
    if (not parent_fsm()->start_ho_preparation(report.target_eci, 0, ev.direct_fwd_path)) {
      trigger(srsran::failure_ev{});
    }
//   } else {
...
{
  srsran::console("Called s1_source_ho_st::handle_ho_cmd\n");
