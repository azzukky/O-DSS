...
void rrc::get_metrics(rrc_metrics_t& m)
{
  if (running) {
    ...
    //printf("Using command_path=%s\n", command_path.c_str());
    FILE* rrc_control_f = fopen(command_path.c_str(), "r");
    if(rrc_control_f) {
      // we know there is a handover that needs to be done, read the data and do it
      // TODO: handle when multiple handovers are available
      handover_command_container_t handover_buffer = {};
      size_t read_data = fread(&handover_buffer, 1, sizeof(handover_command_container_t), rrc_control_f);
      //printf("Read %lu bytes from file\n", read_data);

     // printf("Got command to hand RNTI %hu to eNB %u\n", handover_buffer.rnti, handover_buffer.enb_id);
      // guard condition: only trigger handover for real base stations and RNTIs
      if(handover_buffer.rnti != 0 && handover_buffer.enb_id != 0 && handover_buffer.enb_id != enb_id) {
        // start_custom_handover(handover_buffer.rnti, handover_buffer.enb_id);
      }
      FILE* _i_dont_care = freopen(nullptr, "w", rrc_control_f);
      fclose(rrc_control_f);
    } 
...
/*
Start our custom handover process given a target UE RNTI and a target ENB ID
*/
void rrc::start_custom_handover(uint16_t rnti, uint32_t target_enb) {
  printf("Custom handover initiated (rrc.cc)\n");
 // find the UE we want
 auto target_user = &users[rnti];

 // make sure we get a real, existing user RNTI
 if(target_user == nullptr) {
  printf("Failed to find a UE with RNTI %hu\n", rnti);
  return;
 }

 // trigger our special handover
 (*target_user)->start_custom_s1_handover(target_enb);
}
