...
namespace srsenb
{
  ...
  sched_time_pf::sched_time_pf(const sched_cell_params_t &cell_params_, const sched_interface::sched_args_t &sched_args)
  {
    ...
    agent_prb_path = sched_args.agent_prb_path;
    ...
    sched_save_path = sched_args.sched_save_path;
  }
  ...
  void sched_time_pf::new_tti(sched_ue_list &ue_db, sf_sched *tti_sched)
  {
    ...
    */
    ...
    // save metrics that we need

    ric::sched_metrics_t metrics = {};
    metrics.available_prbs = avail;
    metrics.pending_data = 0;
    metrics.required_prbs = 0;
    metrics.granted_prbs = 0;
    metrics.current_ul_mask = tti_sched->get_ul_mask().to_uint64();
    metrics.ue_rnti = 0;
    // printf("Scheduler sees RNTI of %u\n", metrics.ue_rnti);

    // write metrics to file
    FILE *output_dat = fopen(sched_save_path.c_str(), "a");
    if(output_dat != nullptr) {
      size_t written = fwrite(&metrics, 1, sizeof(ric::sched_metrics_t), output_dat);
      if (written != sizeof(ric::sched_metrics_t))
      {
        printf("Failed to write scheduler data: %lu != %lu\n", written, sizeof(ric::sched_metrics_t));
        perror("Failed writing scheduler data");
      }
      fclose(output_dat);
    }
  }
  ...
  // Function to check if the file size is exactly 8 bytes
  int cnt_ul = 0;
  int wgt_cnt_ul = 0;
  int tti_count = 0;
  ...
  void sched_time_pf::sched_ul_users(sched_ue_list &ue_db, sf_sched *tti_sched)
  {
    ...
    // Write my logic for blanking PRBs
    // auto file_size = std::experimental::filesystem::file_size(agent_prb_path.c_str());
    int low = 0, high = 0;
    // if (file_size == 8){
    std::ifstream file(agent_prb_path.c_str(), std::ios::binary);
    if (!file) {
    ...
    std::ofstream log_file("ue_allocations_good.log", std::ios::app);
    if (log_file.is_open())
    {
      log_file << "time: " << cnt_ul
               << " cnt_ul: " << cnt_ul
               << " Available PRBs: " << avail
               << " Current Mask: " << mask_str << std::endl;
  ...
  uint32_t sched_time_pf::try_ul_alloc(ue_ctxt &ue_ctxt, sched_ue &ue, sf_sched *tti_sched)
  {
    ...
    else
    {
      ...
      if (cnt_ul >= 0)
      {
        ...
        std::ofstream log_file("ue_allocations_strange.log", std::ios::app);
        if (log_file.is_open())
        {
        ...
        ric::sched_metrics_t metrics = {};
        metrics.available_prbs = avail;
        metrics.pending_data = pending_data;
        metrics.required_prbs = pending_rb;
        metrics.granted_prbs = size;
        metrics.current_ul_mask = tti_sched->get_ul_mask().to_uint64();
        metrics.ue_rnti = ue_ctxt.rnti;
        // printf("Scheduler sees RNTI of %u\n", metrics.ue_rnti);
