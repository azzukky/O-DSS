...
void mac::get_metrics(mac_metrics_t& metrics)
{
  ...
  //printf("\tmac::get_metrics sees %lu UEs\n", metrics.ues.size());
  //if(metrics.ues.size() != 0) {
    //printf("\tmac::get_metrics sees demo metric: tx_pkts %d\n", metrics.ues[0].tx_pkts);
  //}
...
void mac::ric_comm(std::map<uint16_t, float>& weights)
{
  ...
  */
  ...
  for (auto& ue_pair : ue_db) {
    ...
    uint8_t cqi = (uint8_t) 0;//ue_metrics.dl_cqi; 
