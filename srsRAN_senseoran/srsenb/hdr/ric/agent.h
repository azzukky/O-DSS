...
#define SCHEDULER_DATA_FILE "/tmp/scheduler_data.dat"
...
struct classification_ue_metrics_t {
  // PROVIDED BY PHY_METRICS_T
  uint16_t      c_rnti; 
  float pusch_sinr; 
  float pucch_sinr;
  float rssi;
  ...
  // PROVIDED BY MAC_UE_METRICS_T <- MAC_METRICS_T <- STACK_METRICS_T
  int      tx_pkts;
  int      tx_errors;
  int      tx_brate;
  int      tx_bler;
  ...
  // uint32_t required_prbs;    // ue.get_required_prb_ul(cc_cfg->enb_cc_idx, pending_data)
  // size_t granted_prbs;    // find_contiguous_ul_prbs(pending_rb, tti_sched->get_ul_mask());
};

struct classification_metrics_t {
  double      timestamp;
  uint32_t    base_station_id;
  // int         available_prbs;

  // list of UEs
  std::vector<classification_ue_metrics_t> *ue_metrics;
};
...
struct sched_metrics_t {
  ...
  uint32_t required_prbs;    // ue.get_required_prb_ul(cc_cfg->enb_cc_idx, pending_data)
  size_t granted_prbs;    // find_contiguous_ul_prbs(pending_rb, tti_sched->get_ul_mask());
  uint64_t current_ul_mask; // ~(tti_sched->get_ul_mask())
  uint16_t ue_rnti; 
};
...
private:
  ...
  void fill_class_metrics();
  size_t serialize_classification_metrics(uint8_t** buffer, classification_metrics_t& metrics);
  ...
  uint8_t iq_buffer[5760000];

  uint32_t enb_id;
  std::string sched_save_path;
  std::string rrc_command_path;
};
